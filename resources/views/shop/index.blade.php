            ...
            <form id="filter_form" action="{{ route('shop.index') }}" method="GET" class="w-full flex flex-col lg:flex-row items-center gap-3 sm:gap-4">
                ...
                <div class="relative w-full lg:w-1/3 flex-shrink-0">
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari gear andalanmu..." 
                        class="w-full bg-white text-black font-montserrat font-bold text-[10px] sm:text-xs rounded-xl sm:rounded-2xl h-[48px] sm:h-[56px] pl-11 sm:pl-12 pr-4 transition-all duration-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-volt placeholder-gray-500"
                    ...
                    <div class="absolute inset-y-0 left-0 pl-4 sm:pl-5 flex items-center pointer-events-none">
                        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
                    </div>
                </div>
                ...
                <div class="w-full lg:w-2/3 flex flex-row items-center gap-3 sm:gap-4">
                    ...
                    <div class="relative flex-grow" id="custom-category-dropdown">
                        ...
                        <!-- WARNA: Solid White -->
                        <button type="button" onclick="toggleCategoryFilter()" class="bg-white text-black font-montserrat font-bold text-[10px] sm:text-xs rounded-xl sm:rounded-2xl flex items-center justify-between w-full h-[48px] sm:h-[56px] px-4 sm:px-5 transition-all duration-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-volt">
                            <div class="flex items-center">
                                <span class="text-gray-500 mr-3">
                                    <svg class="w-4 h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.293A1 1 0 013 6.586V4z"></path></svg>
                                </span>
                                <span id="category_text" class="uppercase text-black">
                                    @php
                        ...
                        <!-- Dropdown Options Box (Solid White) -->
                        <ul id="category_options" class="hidden absolute top-full left-0 w-full mt-2 bg-white rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.8)] py-2 z-[60] custom-scroll max-h-60 overflow-y-auto">
                            <li class="px-5 py-3 text-[10px] sm:text-xs font-bold font-montserrat tracking-widest text-gray-600 hover:text-black hover:bg-gray-100 cursor-pointer transition uppercase" onclick="selectCategoryFilter('', 'SEMUA KATEGORI')">
                                SEMUA KATEGORI
                            </li>
                            @foreach($categories as $category)
                                <li class="px-5 py-3 text-[10px] sm:text-xs font-bold font-montserrat tracking-widest text-gray-600 hover:text-black hover:bg-gray-100 cursor-pointer transition uppercase border-t border-gray-100" onclick="selectCategoryFilter('{{ $category->slug }}', '{{ strtoupper($category->name) }}')">
                    ...
                    <div class="relative flex-shrink-0" id="custom-sort-dropdown">
                        ...
                        <!-- Tombol Kotak Icon (Solid White) -->
                        <button type="button" onclick="toggleSortFilter()" class="bg-white text-black font-montserrat font-bold flex items-center justify-center w-[48px] h-[48px] sm:w-[56px] sm:h-[56px] rounded-xl sm:rounded-2xl transition-all duration-300 shadow-sm focus:outline-none focus:ring-2 focus:ring-volt relative">
                            <svg class="w-5 h-5 sm:w-6 sm:h-6 text-black transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12"></path></svg>
                            
                            <!-- Titik Indikator jika sorting tidak default (Terbaru) -->
                            @if(request('sort') && request('sort') != 'newest')
                                <span class="absolute top-2.5 right-2.5 sm:top-3.5 sm:right-3.5 w-2 h-2 sm:w-2.5 sm:h-2.5 bg-volt rounded-full border border-white"></span>
                            @endif
                        </button>

                        <!-- Dropdown Options Box (Solid White) -->
                        <ul id="sort_options" class="hidden absolute top-full right-0 w-48 sm:w-56 mt-2 bg-white rounded-2xl shadow-[0_15px_40px_rgba(0,0,0,0.8)] py-2 z-[60]">
                            <li class="px-5 py-3 text-[10px] sm:text-xs font-bold font-montserrat tracking-widest text-gray-600 hover:text-black hover:bg-gray-100 cursor-pointer transition uppercase flex items-center justify-between" onclick="selectSortFilter('newest')">
                                TERBARU 
                                @if(request('sort', 'newest') == 'newest') <span class="w-2 h-2 bg-volt rounded-full"></span> @endif
                            </li>
                            <li class="px-5 py-3 text-[10px] sm:text-xs font-bold font-montserrat tracking-widest text-gray-600 hover:text-black hover:bg-gray-100 cursor-pointer transition uppercase border-t border-gray-100 flex items-center justify-between" onclick="selectSortFilter('price_asc')">
                                TERMURAH
                                @if(request('sort') == 'price_asc') <span class="w-2 h-2 bg-volt rounded-full"></span> @endif
                            </li>
                            <li class="px-5 py-3 text-[10px] sm:text-xs font-bold font-montserrat tracking-widest text-gray-600 hover:text-black hover:bg-gray-100 cursor-pointer transition uppercase border-t border-gray-100 flex items-center justify-between" onclick="selectSortFilter('price_desc')">
                                TERMAHAL
                                @if(request('sort') == 'price_desc') <span class="w-2 h-2 bg-volt rounded-full"></span> @endif
                            </li>
                        </ul>
                    </div>
                </div>
