...
<div class="relative w-full min-h-[85vh] rounded-[2.5rem] overflow-hidden bg-white/40 dark:bg-white/[0.02] p-4 sm:p-6 lg:p-8 shadow-sm dark:shadow-[0_8px_32px_rgba(0,0,0,0.3)] border border-gray-200 dark:border-white/10 backdrop-blur-2xl transition-colors duration-500">
    ...
    <div class="relative z-10 space-y-8">
        ...
        <!-- Judul Halaman Dikembalikan ke Original -->
        <div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            @forelse($brands as $brand)
                <div class="bg-white/70 dark:bg-white/[0.03] backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-[1.5rem] overflow-hidden shadow-sm dark:shadow-[0_8px_32px_rgba(0,0,0,0.3)] transition-colors duration-500 group relative">
                    <div class="h-40 bg-black/40 relative shadow-inner overflow-hidden">
                        ...
                        <h4 class="absolute bottom-4 left-4 text-3xl font-bebas text-white tracking-wide z-10">{{ $brand->name }}</h4>
                    </div>
                    
                    <div class="p-5 flex justify-between items-center bg-gray-50/50 dark:bg-transparent">
                        <div>
                            <p class="text-[10px] text-gray-500 dark:text-gray-400 font-medium uppercase tracking-widest">Tautan Publik:</p>
                            <a href="{{ route('brand.show', $brand->slug) }}" target="_blank" class="text-xs text-emerald-600 dark:text-volt hover:underline font-bold tracking-wide">/brand/{{ $brand->slug }}</a>
                        </div>
                        <form action="{{ route('admin.brands.destroy', $brand->id) }}" method="POST" onsubmit="return confirm('Hapus halaman brand ini? (Produk tidak akan terhapus)');">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="bg-red-500/10 hover:bg-red-500 text-red-500 hover:text-white p-2.5 rounded-xl border border-red-500/20 transition">
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        // Fungsi Memproses dan Menampilkan File
        ...
        // Event saat file dipilih manual lewat klik
        ...
        dropzone.addEventListener('drop', (e) => {
            ...
            fileInput.files = files; // Sinkronkan file yang di-drop ke tag input
            handleFiles(files);
        });
    });
</script>
