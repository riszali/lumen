        ...
        <div class="bg-white/70 dark:bg-white/[0.03] backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-[2rem] shadow-sm dark:shadow-[0_8px_32px_0_rgba(0,0,0,0.4)] p-6 sm:p-8 transition-colors duration-500">
            <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 font-montserrat">
                    ...
                    <div class="relative z-40">
                        ...
                        <div id="existing_brand_wrapper" class="relative">
                            ...
                            <ul id="brand_options" class="hidden absolute top-full left-0 w-full mt-2 bg-white/95 dark:bg-[#111111]/95 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl shadow-lg dark:shadow-[0_8px_32px_rgba(0,0,0,0.6)] max-h-72 overflow-y-auto py-2 transition-colors duration-500">
                                ...
                                <div class="px-5 py-2 mt-2 text-[10px] font-bold uppercase tracking-widest text-emerald-600 dark:text-volt border-y border-gray-100 dark:border-white/5 bg-gray-50/50 dark:bg-black/40">Popular Brands</div>
                                @foreach(['Babolat', 'Bullpadel', 'HEAD', 'Oxdog', 'Siux', 'Nox', 'StarVie', 'Nike', 'Adidas', 'Asics', 'Mizuno', 'SiS GO', 'Optimum Nutrition'] as $b)
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <!-- Normal Price -->
                    <div>
                        <label class="block text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-2 font-bold transition-colors duration-500">Harga Normal (Rp) *</label>
                        <input type="hidden" name="price" id="price_raw" value="{{ old('price') }}">
                        <input type="text" id="price_formatted" value="{{ old('price') ? number_format(old('price'), 0, ',', '.') : '' }}" required class="w-full bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm dark:shadow-inner p-3.5 focus:ring-emerald-500 dark:focus:ring-volt text-sm text-gray-900 dark:text-white placeholder-gray-400 transition-all outline-none" oninput="formatRupiah(this, 'price_raw')">
                        @error('price') <span class="text-red-500 dark:text-red-400 text-xs mt-1 font-semibold block">{{ $message }}</span> @enderror
                    </div>

                    <!-- Discount Price -->
                    <div>
                        <label class="block text-[10px] uppercase tracking-widest text-emerald-600 dark:text-volt mb-2 font-bold transition-colors duration-500">Harga Diskon (Rp) - Opsional</label>
                        <input type="hidden" name="discount_price" id="discount_raw" value="{{ old('discount_price') }}">
                        <input type="text" id="discount_formatted" value="{{ old('discount_price') ? number_format(old('discount_price'), 0, ',', '.') : '' }}" class="w-full bg-emerald-50/30 dark:bg-volt/5 border border-emerald-200 dark:border-volt/30 rounded-2xl shadow-sm dark:shadow-inner p-3.5 focus:ring-emerald-500 dark:focus:ring-volt text-sm text-gray-900 dark:text-white placeholder-emerald-300 dark:placeholder-volt/40 transition-all outline-none" placeholder="Kosongkan jika tidak diskon" oninput="formatRupiah(this, 'discount_raw')">
                    ...
                    <div>
                        ...
                        <input type="number" name="stock" value="{{ old('stock', 0) }}" required min="0" class="w-full bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm dark:shadow-inner p-3.5 focus:ring-emerald-500 dark:focus:ring-volt text-sm text-gray-900 dark:text-white transition-all outline-none">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <textarea name="description" rows="6" required placeholder="Jelaskan deskripsi umum produk ini..." class="w-full bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm dark:shadow-inner p-3.5 focus:ring-emerald-500 dark:focus:ring-volt text-sm text-gray-900 dark:text-white placeholder-gray-400 transition-all outline-none">{{ old('description') }}</textarea>
                    ...
                    <div>
                        ...
                        <textarea name="specification" rows="6" placeholder="Contoh:&#10;Berat: 365g&#10;Material: 12K Carbon" class="w-full bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl shadow-sm dark:shadow-inner p-3.5 focus:ring-emerald-500 dark:focus:ring-volt text-sm text-gray-900 dark:text-white placeholder-gray-400 transition-all outline-none">{{ old('specification') }}</textarea>
                        @error('specification') <span class="text-red-500 dark:text-red-400 text-xs mt-1 font-semibold block">{{ $message }}</span> @enderror
                    </div>
                </div>

                <!-- DRAG & DROP MULTI-IMAGE GALLERY -->
                <div class="bg-gray-50/50 dark:bg-black/10 rounded-2xl border border-gray-200 dark:border-white/5 p-6 transition-colors duration-500">
                    <label class="block text-[10px] uppercase tracking-widest text-gray-500 dark:text-gray-400 mb-2 font-bold">Product Images Gallery *</label>
                    <p class="text-[10px] font-medium text-gray-400 dark:text-gray-500 mb-4">Seret & lepas foto sekaligus, atau klik kotak di bawah. <strong class="text-emerald-600 dark:text-volt">Klik pada hasil gambar untuk menjadikannya foto utama (Primary).</strong></p>
                    
                    <input type="hidden" name="primary_image_index" id="primary_image_index" value="0">
                    
                    <!-- Input aslinya disembunyikan -->
                    <input type="file" name="images[]" id="images_input" multiple accept="image/*" required class="hidden">
                    ...
                    <div id="multi_dropzone" onclick="document.getElementById('images_input').click()" class="relative border-2 border-dashed border-gray-300 dark:border-white/20 rounded-2xl p-8 text-center hover:border-emerald-500 dark:hover:border-volt hover:bg-emerald-50/30 dark:hover:bg-volt/5 transition-all duration-300 cursor-pointer mb-4">
                        <div class="pointer-events-none flex flex-col items-center justify-center">
                            ...
                            <span class="text-xs font-bold uppercase tracking-widest text-gray-500 dark:text-gray-400">Klik di sini atau Seret Gambar</span>
                        </div>
                    </div>

                    @error('images') <span class="text-red-500 dark:text-red-400 text-xs font-semibold block mb-2">{{ $message }}</span> @enderror

                    <!-- Container Preview -->
                    <div id="image_preview_container" class="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-5 gap-4">
                ...
                <div class="flex flex-col sm:flex-row gap-6 sm:gap-8 p-5 bg-gray-50/50 dark:bg-black/10 rounded-2xl border border-gray-200 dark:border-white/5 transition-colors duration-500">
                    <label class="flex items-center cursor-pointer group">
                        <input type="checkbox" name="is_active" value="1" checked class="w-5 h-5 rounded border-gray-300 dark:border-white/20 bg-white dark:bg-black/40 text-emerald-500 dark:text-volt shadow-sm focus:ring-emerald-500 dark:focus:ring-volt transition cursor-pointer">
                    ...
                    <label class="flex items-center cursor-pointer group">
                        <input type="checkbox" name="is_featured" value="1" class="w-5 h-5 rounded border-gray-300 dark:border-white/20 bg-white dark:bg-black/40 text-emerald-500 dark:text-volt shadow-sm focus:ring-emerald-500 dark:focus:ring-volt transition cursor-pointer">
...
<script>
    ...
    dropzone.addEventListener('drop', (e) => {
        ...
        if(files.length > 0) {
            fileInput.files = files; // Set input files ke drag&drop
