...
<div class="relative w-full min-h-screen bg-[var(--dark)] bg-noise overflow-hidden pt-navbar pb-24 font-montserrat text-white">
    ...
    <div class="absolute bottom-[-10%] right-[-5%] w-[30%] h-[40%] bg-[#ccff00] rounded-full mix-blend-screen filter blur-[150px] opacity-10 pointer-events-none z-0"></div>
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 relative z-20">
        ...
        <div class="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-white/10 pb-8">
            <div>
                <a href="{{ route('orders.index') }}" class="text-[10px] font-bold uppercase tracking-widest text-gray-500 hover:text-volt-custom mb-4 inline-flex items-center transition duration-300">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg>
                    Kembali ke Pesanan
                </a>
                <h1 class="font-bebas text-5xl sm:text-6xl text-white drop-shadow-md tracking-wide mt-2">ORDER <span class="text-volt-custom">#{{ $order->order_number }}</span></h1>
                        ...
                        <div class="flex flex-col sm:flex-row gap-6 p-4 bg-black/20 rounded-2xl border border-white/5 shadow-inner group">
                            ...
                            <div class="flex-grow flex flex-col justify-center">
                                <h4 class="text-lg font-bebas text-white tracking-wide mb-1">
                                    @if($item->product)
                                        <a href="{{ route('shop.show', $item->product->slug) }}" class="hover:text-volt-custom transition">{{ $item->product_name }}</a>
                                    @else
                                        {{ $item->product_name }}
                                    @endif
                                </h4>
                                ...
                                <div class="mt-auto flex justify-between items-center border-t border-white/10 pt-3">
                                    ...
                                    <p class="text-sm font-bold text-volt-custom">Rp {{ number_format($item->price * $item->quantity, 0, ',', '.') }}</p>
                                </div>
                            </div>
                        </div>
                        @endforeach
                ...
                <div class="bg-white/[0.03] backdrop-blur-2xl border border-white/10 rounded-[2.5rem] p-8 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)]">
                    ...
                    <div class="space-y-4 text-sm font-light tracking-wide">
                        ...
                        <div class="flex justify-between items-center">
                            ...
                            <span class="badge-volt-custom font-bold uppercase text-[10px] tracking-widest px-3 py-1 rounded-full">Gratis</span>
                        </div>
                        <div class="border-t border-white/10 pt-6 mt-4 flex justify-between items-end">
                            ...
                            <span class="font-bebas text-4xl text-volt-custom tracking-wider drop-shadow-md">Rp {{ number_format($order->total_amount, 0, ',', '.') }}</span>
                        </div>
                    </div>
                </div>
